...
| `upload FILE SELECTOR [TAB]` | set FILE on a page's file input (`DOM.setFileInputFiles` + the `input`/`change` events its JS listens for) — drives upload UIs that have no API, e.g. a GitHub repo's social-preview image; Linux-side files are staged to the Windows side automatically |
